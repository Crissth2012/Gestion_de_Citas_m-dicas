<template>
  <div>
    ...
    <b-table :data="administradores" bordered>
      <b-table-column v-slot="props" label="Cédula">
        {{ props.row.cedula }}
      </b-table-column>
...
export default {
  ...
  methods: {
    async iniciar () {
      try {
        this.administradores = await this.$axios.$get('/api/administradores')
      } catch (err) {
        console.error(err)
      }
    },
    async eliminar (row) {
      if (!confirm('¿Seguro que desea eliminar el administrador?')) { return }
      try {
        await this.$axios.$delete('/api/administradores/' + row._id)
        await this.iniciar()
      } catch (err) {
        console.error(err)
      }
