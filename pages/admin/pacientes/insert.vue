...
export default {
  ...
  async mounted () {
    if (this.$route.params.id) {
      const paciente = await this.$axios.$get('/api/pacientes/' + this.$route.params.id)
  ...
  methods: {
    async enviarFormulario () {
      try {
        if (this.$route.params.id) {
          await this.$axios.$put('/api/pacientes/' + this.$route.params.id, this.formulario)
        } else {
          await this.$axios.$post('/api/pacientes', this.formulario)
        }
        this.$router.push('/admin/pacientes')
      } catch (err) {
        showError(err)
      }
    }
